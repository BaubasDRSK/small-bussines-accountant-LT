...
export default function Settings({ auth, newlist, updateExpenseRoute }) {
    ...
    const [searchName, setSearchName] = useState(localStorage.getItem('searchExpenseName')) ?? '';
    ...
    const [sort, setSort] = useState({ sortDirection: 'desc', sortName: 'expense_date' });
    ...
    useEffect(() => {
        ...
        localStorage.setItem('searchExpenseName', searchName);
    ...
    const doSort = n => {
        setSort(s => {
            ...
            switch (s.sortDirection) {
                ...
                default: return { sortDirection: 'asc', sortName: 'expense_date' };
            };
        });
    }
    ...
    useEffect(() => {
        ...
        if (expensesList !== null || searchName === '' && pagination === 15) {
            uzklausa(newlist, searchName, sort);
        }
    }, [sort]);
                    ...
                    <section className="w-full max-w-full">
                        <div className="mb-6">
                            ...
                            <p className="mt-2 text-gray-500 dark:text-gray-400">
                                View, filter, and manage all business expenses.
                            </p>
                        </div>
                        ...
                        <div className="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg border border-gray-200 dark:border-gray-700">
                            ...
                            <div className='flex flex-col md:flex-row justify-between w-full p-4 border-b dark:border-gray-700'>
                                <div className="flex mb-4 md:mb-0">
                                    <input
                                        ...
                                        type="text"
                                        placeholder="Search by expense nr. or name or company"
                            ...
                            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                    <tr>
                                        ...
                                        <th scope="col" className="px-6 py-3 cursor-pointer select-none hover:bg-gray-100 dark:hover:bg-gray-600/70 transition duration-150" onClick={() => doSort('expense_date')}>
                                            <div className="flex items-center">
                                                <div className="flex flex-col">
                                                    <p className="font-semibold">Expense number</p>
                                                    <p className="text-xs font-normal text-gray-400">Issue date</p>
                                                </div>
                                                <SortIcon name="expense_number" />
                                            </div>
                                        </th>
                                ...
                                <tbody>
                                    {expensesList !== null ? (expensesList.data.map((item) => (
                                        <tr
                                            key={item.id}
                                            onClick={() => window.location.href = '/expenses/show/' + item.id}
                                        ...
                                        >
                                            <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                <p className="text-base font-semibold text-blue-600 dark:text-blue-400">{item.expense_number}</p>
                                                <p className="text-xs text-gray-600 dark:text-gray-400">{item.expense_date}</p>
                                                {/* Mobile summary */}
                                        ...
                                        <tr>
                                            <td colSpan="5" className="px-6 py-4 text-center text-base font-medium text-gray-900 dark:text-gray-300">Loading expenses...</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                            ...
                            <div className="flex justify-center items-center py-4 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-700">
                                {expensesList !== null ? (expensesList.links.map((item) => (
                                    <button
                                        key={item.label}
